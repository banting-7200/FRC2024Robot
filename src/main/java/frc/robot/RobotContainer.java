...
public class RobotContainer {
    ...
    //Runs the pickup routine on button press. 
    //The routine runs the intake, then once complete tucks the arm, moves it to the tuck position 
    //and finally tells the lights that we are now carrying a note if the arm movementwas completed successfully and not interrupted.
    new JoystickButton(CoPilotController, copilotController.pickupButton)
        .onTrue(
            new intakeCommand(
                Shooter.intakeRPM,
    ...
    //Simply toggles the hook up or down on a button press.
    new JoystickButton(CoPilotController, copilotController.hookButton)
        .onTrue(Commands.runOnce(() -> arm.toggleHook()));

    //This is a backup binding in case the intake routine fails. 
    //On button press it stops the intake motor, then tucks the arm like in the regular intake routine,
    //first tucking the arm, then moving to the tuck position, and finally setting the light state if moved successfully.
    new JoystickButton(CoPilotController, copilotController.carryButton)
        .onTrue(
            Commands.runOnce(() -> shooter.stopIntakeMotor())
                .andThen(
    ...
    //A button binding with manual control over the state of the shooter solenoid to tuck and untuck it at will incase something goes wrong.
    new JoystickButton(CoPilotController, copilotController.extendButton)
        .onTrue(Commands.runOnce(() -> arm.toggleShooterState()));
